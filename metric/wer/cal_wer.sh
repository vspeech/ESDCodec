...
python get_wav_res_ref_text.py $meta_lst $output_dir $wav_wav_text

timestamp=$(date +%s)
thread_dir=thread_metas_$timestamp/
mkdir $thread_dir
num_job=8
...
out_dir=thread_metas_$timestamp/results/
mkdir $out_dir
...
if [ ${num_job_minus_1} -ge 0 ];then
	for rank in $(seq 0 $((num_job - 1))); do
		...
		CUDA_VISIBLE_DEVICES=$rank python run_wer.py $thread_dir/thread-0$rank.lst $sub_score_file $lang &
	done
fi
wait
...
python average_wer.py $out_dir/merge.out $score_file
