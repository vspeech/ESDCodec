...
def _build_semantic_model(
    ...
    **kwargs,
    ...
    Inputs:
    - esdcodec_path: str, path to the esdcodec model
    ...
    # load mean and std
    meanvar_path = os.path.join(esdcodec_path, meanvar_fname)
...
class Inference:
    """
    Inference class for ESDCodec.
    """

    def __init__(
        self,
        esdcodec_model,
        esdcodec_path="hf://amphion/esdcodec",
        ...
        **kwargs,
        ...
        """
        Inputs:
        - esdcodec_model: ESDCodec instance, the model weight is loaded by safetensors
        - esdcodec_path: str, path to the esdcodec model
        - w2v_path: str, path to the w2v-bert model
        - device: str, device to run the model
        - autocast: bool, whether to use autocast to fp16 for model inference
        """
        esdcodec_path = cached_path(esdcodec_path)
    ...
    def encode(
        ...
        with ctx:
            semantic_codes, acoustic_codes = self.model.encode(
                audio, num_quantizers=n_quantizers
            )
