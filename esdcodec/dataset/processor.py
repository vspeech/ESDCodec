...
def url_opener(data, mode="train"):
...
def tar_file_and_group(data, mode="train"):
                ...
                with stream.extractfile(tarinfo) as file_obj:
                    try:
                        if postfix == 'txt':
                            ...
                            info = file_obj.read().decode('utf8')
                            json_f = json.loads(info)
                            #print("info: ", json_f)
                            example['duration'] = json_f['duration']
                            example['language'] = json_f['language']
                            #text = json_f['text'].strip()
                            #if example['language'] == 'zh':
                            #    example['text'] = ast.literal_eval(f'"{text}"')
                            #else:
                            #    example['text'] = json_f['text'].strip()
                    except Exception as ex:
                        valid = False
...
def gluster_filter(
    ...
    for sample in data:
        ...
        if (new_sample["speech"].shape[-1] // new_sample["sample_rate"]) > 45.0:
            ...
            continue
        if (new_sample["speech"].shape[-1] // new_sample["sample_rate"]) < 1.0:
            print("too short audio, skipped")
            continue
