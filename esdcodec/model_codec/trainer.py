...
from esdcodec.utils.base_trainer import BaseTrainer
import safetensors
import numpy as np
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, args=None, cfg=None, **kwargs):
        ...
        from .loss import GANLoss, MelSpectrogramLoss, MultibandMelSpectrogramLoss, MagLoss
        ...
        self.cond_spec_loss = MelSpectrogramLoss(
            pow=2,
            mag_weight=1,
            log_weight=1,
            n_mels=[20, 40, 80],
    ...
    def _train_step(self, batch):
        ...
        cond_out = out_dict.source
        cond_mag = out_dict.source_mag
        ...
        resample6k = AudioSignal(x_wav, 24000).resample(6000)
        cond_spec_loss = self.cond_spec_loss(
            resample6k, AudioSignal(cond_out, 6000)
        )
        linear_mag = resample6k.stft(window_length=960, hop_length=240, match_stride=True)
        mag = torch.abs(linear_mag)
        cond_mag_loss1, cond_mag_loss2 = self.cond_mag_loss(cond_mag, mag.squeeze(1))
        cond_mag_loss = cond_mag_loss1 + cond_mag_loss2
        total_loss = (
        ...
        # ---------- Generator training ----------------
        if semantic_edict:
            total_loss += self.cfg.lambda_semantic_commitment_loss * semantic_edict["penalty"]
            metrics.update(
                {
        ...
        if self.distill:
            ...
            feat = torch.nn.functional.avg_pool1d(
                feat, self.model_module.semantic_downsample_factor, self.model_module.semantic_downsample_factor
            )
            distill_loss = F.mse_loss(
                feat, semantic_edict["x"][..., : feat.shape[-1]]
            )
        ...
        metrics.update(
            {
                ...
                "spec_loss": spec_loss,
                "cond_spec_loss": cond_spec_loss,
                "cond_mag_loss": cond_mag_loss,
                "feat_loss": feat_loss,
